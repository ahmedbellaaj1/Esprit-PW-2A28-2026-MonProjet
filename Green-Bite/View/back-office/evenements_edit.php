<?php
/**
 * Green-Bite Back-Office - Modifier un Événement
 */
...
if (!$eventData) { header('Location: /Green-Bite/View/back-office/evenements.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $event = new Evenement($titre, $description, $date_event, $lieu, $type, $organisateur_id);
...
// Pre-fill from DB
$formData = $_SERVER['REQUEST_METHOD'] === 'POST' ? $_POST : $eventData;
...
<head>
    ...
    <title>Modifier Événement - GreenBite Admin</title>
...
<div class="dashboard-layout">
    ...
    <div class="dashboard-main"><div class="page-content">
        <div class="page-header">
            ...
            <p><?= htmlspecialchars($eventData['titre']) ?></p>
        </div>
            ...
            <form method="POST">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div class="ev-field">
                        ...
                        <input type="date" id="date_event" name="date_event" value="<?= htmlspecialchars($formData['date_event'] ?? '') ?>">
                    </div>
                    <div class="ev-field">
                        <label for="lieu">📍 Lieu *</label>
                        <input type="text" id="lieu" name="lieu" value="<?= htmlspecialchars($formData['lieu'] ?? '') ?>">
                    </div>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div class="ev-field">
                    ...
                    <div class="ev-field">
                        ...
                        <select id="organisateur_id" name="organisateur_id">
                            <?php foreach($organisateurs as $org): ?>
                                <option value="<?= $org['id'] ?>" <?= ($formData['organisateur_id'] ?? 0) == $org['id'] ? 'selected' : '' ?>><?= htmlspecialchars($org['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                <div class="ev-form-actions">
                    <button type="submit" class="primary-btn">💾 Sauvegarder les modifications</button>
