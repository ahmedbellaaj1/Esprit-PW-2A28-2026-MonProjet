...
// ── État vérification IA image
let donImageVerified = false;  // true = validé alimentaire (admin modal)
let frontImageVerified = false;  // true = validé alimentaire (front modal)
...
/* ================================================================
   IA IMAGE — VÉRIFICATION ALIMENTAIRE
   ================================================================ */

/**
 * Met à jour le badge visuel d'un modal donné.
 * @param {'admin'|'front'} ctx
 * @param {'pending'|'ok'|'error'|'hidden'} state
 * @param {string} message
 */
function setAIBadge(ctx, state, message = '') {
    const badgeId = ctx === 'admin' ? 'don-ai-badge' : 'front-ai-badge';
    const btnId = ctx === 'admin' ? 'don-btn-save' : 'front-btn-publier';
    ...
    badge.style.cssText = `display:flex;align-items:center;gap:8px;margin-top:10px;
        padding:10px 14px;border-radius:12px;font-size:0.85rem;font-weight:600;
        background:${s.bg};color:${s.color};`;
    badge.innerHTML = `<span style="font-size:1.1rem;">${s.icon}</span><span>${message}</span>`;
...
/**
 * Envoie l'image à l'API PHP qui interroge Gemini.
 * @param {File} file
 * @param {'admin'|'front'} ctx
 */
async function verifyImageWithAI(file, ctx) {
    const previewId = ctx === 'admin' ? 'don-image-preview' : 'front-image-preview';
    const contentId = ctx === 'admin' ? 'don-dropzone-content' : 'front-dropzone-content';
    ...
    if (ctx === 'admin') donImageVerified = false;
    else frontImageVerified = false;

    // Limite taille : max 1 Mo
    if (file.size > 1 * 1024 * 1024) {
        setAIBadge(ctx, 'error', '⚠️ Image trop lourde pour l\'analyse IA (max 1 Mo). Compressez l\'image et réessayez.');
        return;
    }

    setAIBadge(ctx, 'pending', 'Analyse IA en cours…');

    if (json.is_food) {
        if (ctx === 'admin') donImageVerified = true;
        else frontImageVerified = true;
        setAIBadge(ctx, 'ok', json.message || 'Produit alimentaire confirmé ✅');
    } else {
        const msg = json.message || json.label || '';
        const isQuota = msg.toLowerCase().includes('quota') || msg.toLowerCase().includes('limite');
        if (isQuota) {
            setAIBadge(ctx, 'pending', '⏳ Quota API dépassé — patientez 1 minute puis réessayez.');
        } else {
            setAIBadge(ctx, 'error', msg || 'Produit non alimentaire ❌ — Don refusé.');
        }
    }

    // Admin modal — gestionnaires fichier
    function handleDonImageSelect(file) {
        if (file) verifyImageWithAI(file, 'admin');
    }
    function handleDonImageDrop(event) {
        event.preventDefault();
        document.getElementById('don-dropzone').style.borderColor = '#14b8a6';
        const file = event.dataTransfer.files[0];
        if (file) verifyImageWithAI(file, 'admin');
    }

    // Front modal — gestionnaires fichier
    function handleFrontImageSelect(file) {
        if (file) verifyImageWithAI(file, 'front');
    }
    function handleFrontImageDrop(event) {
        event.preventDefault();
        document.getElementById('front-dropzone').style.borderColor = '#14b8a6';
        const file = event.dataTransfer.files[0];
        if (file) verifyImageWithAI(file, 'front');
    }

    /** Réinitialise la zone image d'un modal */
    function resetImageZone(ctx) {
        const previewId = ctx === 'admin' ? 'don-image-preview' : 'front-image-preview';
        const contentId = ctx === 'admin' ? 'don-dropzone-content' : 'front-dropzone-content';
        const inputId = ctx === 'admin' ? 'don-image-input' : 'front-image-input';
    ...
    async function loadDonStats() {
        try {
            ...
            const recup = s['récupéré'] || 0;
            const pourcentageRecup = total > 0 ? Math.round((recup / total) * 100) : 0;

            el.innerHTML = `
            <div class="metric-card">
                <div class="metric-icon icon-teal">🎁</div>
                <div class="metric-value">${s.disponible || 0}</div>
                <div class="metric-label">Disponibles</div>
            </div>
            <div class="metric-card">
                <div class="metric-icon icon-amber">🔒</div>
                <div class="metric-value" style="color:#ca8a04;">${s['réservé'] || 0}</div>
                <div class="metric-label">Réservés</div>
            </div>
            <div class="metric-card">
                <div class="metric-icon icon-green">📦</div>
                <div class="metric-value" style="color:#0f766e;">${recup}</div>
                <div class="metric-label">Récupérés</div>
                <div style="font-size:0.7rem;color:#16a34a;margin-top:4px;">📊 ${pourcentageRecup}% du total</div>
            </div>
            <div class="metric-card">
                <div class="metric-icon icon-red">⚠️</div>
                <div class="metric-value" style="color:#991b1b;">${s['périmé'] || 0}</div>
                <div class="metric-label">Périmés</div>
            </div>
            <div class="metric-card">
    ...
    async function openDonModal(donData = null) {
        ...
        // Réinitialiser la zone image uniquement pour un nouveau don
        if (!donData) {
            resetImageZone('admin');
        } else {
            // En mode édition, l'image n'est pas obligatoire
            donImageVerified = true;
            setAIBadge('admin', 'hidden');
            const imgSection = document.getElementById('don-image-section');
        ...
        if (donData) {
            ...
            // Afficher la zone image pour les nouveaux dons
            const imgSection = document.getElementById('don-image-section');
    ...
    async function saveDon() {
        // Vérification IA obligatoire uniquement pour les nouveaux dons
        if (!editingDonId && !donImageVerified) {
            showAlert('don-alert', '⚠️ Veuillez uploader une photo et attendre la validation IA avant d\'enregistrer.');
            return;
        }
    ...
    function renderFrontStats(dons) {
        ...
        const pourcentageRecup = total > 0 ? Math.round((rec / total) * 100) : 0;
        ...
        <div class="metric-card" style="text-align:center;">
            <div style="font-size:2rem;">✅</div>
            <div class="metric-value" style="color:#16a34a;">${dispo}</div>
            <div class="metric-label">Disponibles</div>
        </div>
        <div class="metric-card" style="text-align:center;">
            <div style="font-size:2rem;">📦</div>
            <div class="metric-value" style="color:#0f766e;">${rec}</div>
            <div class="metric-label">Récupérés</div>
            <div style="font-size:0.7rem;color:#16a34a;margin-top:4px;">📊 ${pourcentageRecup}% du total</div>
        </div>
    `;
    ...
    function openFrontCreateDonModal() {
        ...
        resetImageZone('front');
        frontRenderProduitLines([{ nom_produit: '', quantite: '', date_peremption: '' }]);
        const btn = document.getElementById('front-btn-publier');
    ...
    async function submitFrontDon() {
        ...
        if (!frontImageVerified) {
            showAlert('front-don-create-alert', '⚠️ Veuillez uploader une photo et attendre la validation IA avant de publier.');
            return;
        }
    ...
    window.frontRemoveProduitLine = frontRemoveProduitLine;
    window.handleFrontImageSelect = handleFrontImageSelect;
    window.handleFrontImageDrop = handleFrontImageDrop;
    ...
    window.handleDonImageSelect = handleDonImageSelect;
    window.handleDonImageDrop = handleDonImageDrop;
