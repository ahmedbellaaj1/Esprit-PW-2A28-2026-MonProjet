...
<<<<<<< HEAD
            // Détection des allergies et préférences dynamiques
            $detected_preference = $this->findDynamicPreference($msg_lower);
            $detected_allergy = $this->findDynamicAllergy($msg_lower);

            if ($detected_preference) {
                $analysis['has_diet_criteria'] = true;
                $analysis['diet_type'] = $detected_preference['nom'];
                $analysis['diet_description'] = $detected_preference['description'];
                $analysis['intent'] = 'diet_preference';
            ...
            if ($detected_allergy) {
                ...
                $analysis['allergy_name'] = $detected_allergy['nom'];
                $analysis['allergy_description'] = $detected_allergy['description'];
=======
...
>>>>>>> e2c825fb4e8f094eb8c5d8bde41073ee13565fcd
    ...
    /**
<<<<<<< HEAD
     * Rechercher une préférence dans la base de données
     */
    private function findDynamicPreference(string $message): ?array
    {
        try {
            $stmt = $this->pdo->query('SELECT nom, description FROM master_preferences');
            $prefs = $stmt->fetchAll();
            foreach ($prefs as $p) {
                if (stripos($message, $p['nom']) !== false) {
                    return $p;
                }
            }
        } catch (Exception $e) {}
    ...
    /**
     * Rechercher une allergie dans la base de données
     */
    private function findDynamicAllergy(string $message): ?array
    {
        try {
            $stmt = $this->pdo->query('SELECT nom, description FROM master_allergies');
            $allergies = $stmt->fetchAll();
            foreach ($allergies as $a) {
                if (stripos($message, $a['nom']) !== false || (stripos($message, 'allergie') !== false && stripos($message, $a['nom']) !== false)) {
                    return $a;
                }
            }
        } catch (Exception $e) {}
...
<<<<<<< HEAD
        $diet_desc = $analysis['diet_description'] ?? null;
        $allergy = $analysis['allergy_name'] ?? null;
        $allergy_desc = $analysis['allergy_description'] ?? null;

        if ($intent === 'diet_preference' && $diet) {
            $resp = "Très bien! 💚 Je vais vous proposer nos meilleurs produits adaptés au régime **$diet**.";
            if ($diet_desc) {
                $resp .= "\n\n💡 *Note : $diet_desc*";
            }
            return $resp;
        ...
        if ($intent === 'allergies' && $allergy) {
            $resp = "C'est noté ! 🏥 Je vais filtrer nos produits pour éviter tout ce qui contient des **$allergy**.";
            if ($allergy_desc) {
                $resp .= "\n\n⚠️ *Information : $allergy_desc*";
            }
            return $resp;
        }

        // Réponses contextuelles génériques
=======
...
>>>>>>> e2c825fb4e8f094eb8c5d8bde41073ee13565fcd
        $responses = [
            ...
            'diet_preference' => [
<<<<<<< HEAD
                "Très bien! 💚 Je vais vous proposer nos meilleurs produits adaptés.",
=======
...
>>>>>>> e2c825fb4e8f094eb8c5d8bde41073ee13565fcd
     ...
     */
<<<<<<< HEAD
    public function getRecommendations(string $message, int $id_message, int $limit = 5, ?int $id_utilisateur = null): array
=======
...
<<<<<<< HEAD
            // Si l'utilisateur est connecté et n'a pas spécifié de régime, on prend son profil par défaut
            if ($id_utilisateur && !$analysis['has_diet_criteria']) {
                $userProfile = $this->getUserHealthProfile($id_utilisateur);
                if ($userProfile) {
                    if ($userProfile['preference_alimentaire']) {
                        $analysis['diet_type'] = $userProfile['preference_alimentaire'];
                        $analysis['has_diet_criteria'] = true;
                    }
                    // Pour les allergies, on les ajoute aux mots-clés de recherche négative ou filtrage
                    if ($userProfile['allergies']) {
                        $analysis['user_allergies'] = $userProfile['allergies'];
                    }
                }
            }
...
<<<<<<< HEAD
                // On cherche dans les catégories du produit
                $conditions[] = '(p.categorie LIKE ? OR p.description LIKE ?)';
                $params[] = '%' . $analysis['diet_type'] . '%';
                $params[] = '%' . $analysis['diet_type'] . '%';
            }

            // Filtre allergies (Exclure les produits contenant l'allergène)
            if (isset($analysis['allergy_name'])) {
                $conditions[] = 'p.nom NOT LIKE ? AND p.description NOT LIKE ?';
                $params[] = '%' . $analysis['allergy_name'] . '%';
                $params[] = '%' . $analysis['allergy_name'] . '%';
            }
            
            if (isset($analysis['user_allergies'])) {
                $userAllergies = explode(',', $analysis['user_allergies']);
                foreach ($userAllergies as $ua) {
                    $ua = trim($ua);
                    if ($ua !== '') {
                        $conditions[] = 'p.nom NOT LIKE ? AND p.description NOT LIKE ?';
                        $params[] = '%' . $ua . '%';
...
>>>>>>> e2c825fb4e8f094eb8c5d8bde41073ee13565fcd
    ...
    /**
<<<<<<< HEAD
     * Récupérer le profil de santé d'un utilisateur
     */
    private function getUserHealthProfile(int $id_utilisateur): ?array
    {
        try {
            $stmt = $this->pdo->prepare('SELECT preference_alimentaire, allergies FROM user_health WHERE id_user = ?');
