<?php
// Controller/DonController.php — Logique métier (CRUD) pour les dons alimentaires
...
class DonController {
     ...
     * @return array { is_food: bool, label: string }
     */
    public function verifyImage(string $imageData, string $mimeType): array {
        if (!defined('GEMINI_API_KEY') || GEMINI_API_KEY === 'VOTRE_CLE_API_GEMINI_ICI') {
            return ['is_food' => false, 'label' => 'Clé API Gemini non configurée.'];
        }
        ...
        if (!$response || $httpCode !== 200) {
            ...
            return ['is_food' => false, 'label' => $msg];
        }
