...
document.getElementById('paymentForm').addEventListener('submit', async (e) => {
    ...
    try {
        if (paymentMethod === 'stripe') {
            // PAIEMENT STRIPE
            await handleStripePayment(address, deliveryMode, deliveryDate);
        } else {
            // PAIEMENT À LA LIVRAISON
            await handleCashPayment(address, deliveryMode, deliveryDate);
        }
    } catch (error) {
...
async function handleStripePayment(address, deliveryMode, deliveryDate) {
    const cart = new Cart();
    const items = cart.getItems();
    const total = items.reduce((sum, item) => sum + (item.prix * item.quantite), 0);
    ...
    const { paymentIntent, error } = await stripe.confirmCardPayment(piData.client_secret, {
        payment_method: {
            card: cardElement,
            billing_details: {email: 'user@greenbite.local'}
        }
    });
    ...
    if (paymentIntent.status === 'succeeded') {
        await createOrder(address, deliveryMode, deliveryDate, 'stripe', paymentIntent.id);
        showSuccess();
    } else {
...
async function handleCashPayment(address, deliveryMode, deliveryDate) {
    await createOrder(address, deliveryMode, deliveryDate, 'cash', null);
    showSuccess();
}

async function createOrder(address, deliveryMode, deliveryDate, method, paymentId) {
    const cart = new Cart();
    const items = cart.getItems();
