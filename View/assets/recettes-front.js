...
<<<<<<< HEAD
  // Detection Allergène
  const userAllergy = (window.__USER_ALLERGY || '').toLowerCase().trim();
  const ingredients = (window.__FRONT_INGREDIENTS_BY_RECIPE || {})[String(recipe.id_recette)] || [];
  let foundAllergen = null;
  
  if (userAllergy) {
    foundAllergen = ingredients.find(ing => (ing.nom || '').toLowerCase().includes(userAllergy));
  }

  // UI Allergène Warning
  const modalBox = document.querySelector('.modal-box');
  const existingWarning = modalBox.querySelector('.allergy-warning-banner');
  if (existingWarning) existingWarning.remove();

  if (foundAllergen) {
    const warning = document.createElement('div');
    warning.className = 'allergy-warning-banner';
    warning.style.cssText = 'background: #fef2f2; border: 2px solid #ef4444; border-radius: 16px; padding: 1.25rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 1rem; color: #991b1b; animation: shake 0.5s ease-in-out;';
    warning.innerHTML = `
      <span style="font-size: 2rem;">⚠️</span>
      <div>
        <div style="font-weight: 800; font-size: 1.05rem;">Système de sécurité allergique</div>
        <div style="font-size: 0.95rem;">Ce plat contient des <strong>${foundAllergen.nom}</strong> (Allergène détecté : ${userAllergy}).</div>
      </div>
    `;
    // Add shake animation
    if (!document.getElementById('allergy-shake-style')) {
      const s = document.createElement('style');
      s.id = 'allergy-shake-style';
      s.textContent = '@keyframes shake { 0%, 100% {transform: translateX(0);} 25% {transform: translateX(-5px);} 75% {transform: translateX(5px);} }';
      document.head.appendChild(s);
    }
    modalBox.insertBefore(warning, modalBox.querySelector('.modal-header').nextSibling);
  }
...
<<<<<<< HEAD
  window.checkAllergensOnCards = function() {
    const userAllergy = (window.__USER_ALLERGY || '').toLowerCase().trim();
    if (!userAllergy) return;

    document.querySelectorAll('.recette-card').forEach((card) => {
      const id = card.dataset.id;
      const ingredients = (window.__FRONT_INGREDIENTS_BY_RECIPE || {})[String(id)] || [];
      const hasAllergen = ingredients.some(ing => (ing.nom || '').toLowerCase().includes(userAllergy));

      const imgDiv = card.querySelector('.product-img');
      const existingBadge = imgDiv.querySelector('.allergy-badge');
      ...
      if (hasAllergen) {
        ...
        badge.textContent = '⚠️';
        badge.title = 'Attention : Contient un allergène (' + userAllergy + ')';
        badge.style.cssText = 'position:absolute;top:8px;left:8px;background:#ef4444;color:white;padding:4px 8px;border-radius:6px;font-size:1.1rem;font-weight:bold;z-index:10;box-shadow:0 2px 8px rgba(239, 68, 68, 0.4);';
